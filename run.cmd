...
for /d %%i in (%SDK_DIR%\*) do (
    ...
    7z a %TRAVIS_BUILD_DIR%\%PREFIX%-%%~nxi.zip %%i/*
)
...
:package_msvc_sdk
    ...
    xcopy "%VCINSTALLDIR%\Tools\MSVC\%VCTOOLS_VERSION%\lib" "%SDK_DIR%\lib" /s /e /h /y /i /k
...
:package_windows_sdk
    ...
    for /d %%i in (%WINDOWS_SDK%\Include\*) do (
        set WINDOWS_SDK_VER=%%~nxi
        echo %WINDOWS_SDK_VER%
        xcopy %%i "%SDK_DIR%\Windows-SDK-%WINDOWS_SDK_VER%-includes" /s /e /h /y /i /k
    )
